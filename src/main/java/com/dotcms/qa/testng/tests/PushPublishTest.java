...
public class PushPublishTest {
	...
	//test540
	private String test540folderName6="test540";
	//test541
	private String test541folderName7="test541";
	private String test541folderName72="test5412";
	private String test541contentStructureName20="File Asset";
	private String test541contentStructureName20Field1="title";
	private String test541contentTitle20="test-514";
	...
	private void deletePreviousTest() throws Exception{
		...
		try{
			...
			if(browserPage.doesFolderExist(test540folderName6)){
				browserPage.deleteFolder(test540folderName6);
			}

			if(browserPage.doesFolderExist(test541folderName7)){
				browserPage.deleteFolder(test541folderName7);
			}
	 ...
	 */
	/**
	 * Add new folder and push
	 * http://qa.dotcms.com/index.php?/cases/view/540
	 * @throws Exception
	 */
	@Test (groups = {"PushPublishing"})
	public void tc540_CreateNewFolderAndPush() throws Exception{
		try{
			//Calling authoring Server
			IPortletMenu portletMenu = callAuthoringServer();

			ISiteBrowserPage browserPage = portletMenu.getSiteBrowserPage();
			browserPage.createFolder(null, test540folderName6);

			Assert.assertTrue(browserPage.doesFolderExist(test540folderName6), "ERROR - Folder ('"+test540folderName6+"') should exist in authoring server");
			//push folder
			browserPage.pushFolder(test540folderName6);

			IPublishingQueuePage publishingQueuePage = portletMenu.getPublishingQueuePage();
			//wait until 5 minutes to check if the content was pushed
			boolean isPushed = publishingQueuePage.isObjectBundlePushed(test540folderName6,5000,60);
			Assert.assertTrue(isPushed, "ERROR - Authoring Server: Folder ("+test540folderName6+") push should not be in pending list.");

			//delete folder
			browserPage = portletMenu.getSiteBrowserPage();
			browserPage.deleteFolder(test540folderName6);
			Assert.assertFalse(browserPage.doesFolderExist(test540folderName6), "ERROR - Folder ('"+test540folderName6+"') should not exist in authoring server");
			logoutAuthoringServer();
			...
			browserPage = portletMenu.getSiteBrowserPage();
			Assert.assertTrue(browserPage.doesFolderExist(test540folderName6), "ERROR - Folder ('"+test540folderName6+"') should exist in receiver server");
	...
	/**
	 * Edit a folder that exists on both servers and push again
	 * http://qa.dotcms.com/index.php?/cases/view/541
	 * @throws Exception
	 */
	@Test (groups = {"PushPublishing"})
	public void tc541_EditFolderAndPushAgain() throws Exception{
		try{
			//Calling authoring Server
			IPortletMenu portletMenu = callAuthoringServer();

			ISiteBrowserPage browserPage = portletMenu.getSiteBrowserPage();
			browserPage.createFolder(null, test541folderName7);

			Assert.assertTrue(browserPage.doesFolderExist(test541folderName7), "ERROR - Folder ('"+test541folderName7+"') should exist in authoring server");
			//push folder
			browserPage.pushFolder(test541folderName7);

			IPublishingQueuePage publishingQueuePage = portletMenu.getPublishingQueuePage();
			//wait until 5 minutes to check if the content was pushed
			boolean isPushed = publishingQueuePage.isObjectBundlePushed(test541folderName7,5000,60);
			Assert.assertTrue(isPushed, "ERROR - Authoring Server: Folder ("+test541folderName7+") push should not be in pending list.");
			logoutAuthoringServer();
			...
			browserPage = portletMenu.getSiteBrowserPage();
			Assert.assertTrue(browserPage.doesFolderExist(test541folderName7), "ERROR - Folder ('"+test541folderName7+"') should exist in receiver server");
			logoutReceiverServer();
			...
			browserPage = portletMenu.getSiteBrowserPage();
			IFolderAddOrEditPage folder = browserPage.editFolder(test541folderName7);
			folder.setTitle(test541folderName72);
			folder.setName(test541folderName72);
			folder.save();

			browserPage = portletMenu.getSiteBrowserPage();
			Assert.assertTrue(browserPage.doesFolderExist(test541folderName72), "ERROR - Folder ('"+test541folderName72+"') should exist in authoring server");

			IContentAddOrEdit_ContentPage contentPage = browserPage.addFileInFolder(test541folderName72, test541contentStructureName20);

			List<Map<String,Object>> fields = new ArrayList<Map<String, Object>>();
			Map<String,Object> map = new HashMap<String,Object>();
			map.put("type", WebKeys.TEXT_FIELD);
			map.put(test541contentStructureName20Field1, test541contentTitle20);
			fields.add(map);
			map = new HashMap<String,Object>();
			map.put("type", WebKeys.BINARY_FIELD);
			map.put(test541contentStructureName20Field2, test541contentTextArea20);
			fields.add(map) ;
			contentPage.setFields(fields);
			contentPage.sleep(2);
			contentPage.saveAndPublish();
			...
			//push folder
			browserPage = portletMenu.getSiteBrowserPage();
			browserPage.pushFolder(test541folderName72);

			publishingQueuePage = portletMenu.getPublishingQueuePage();
			//wait until 5 minutes to check if the content was pushed
			isPushed = publishingQueuePage.isObjectBundlePushed(test541folderName72,5000,60);
			Assert.assertTrue(isPushed, "ERROR - Authoring Server: Folder ("+test541folderName72+") push should not be in pending list.");

			//delete folder
			IContentSearchPage contentSearchPage = portletMenu.getContentSearchPage();
			contentSearchPage.unpublish(test541fileName20, test541contentStructureName20);
			contentSearchPage.archive(test541fileName20, test541contentStructureName20);
			contentSearchPage.delete(test541fileName20, test541contentStructureName20);
			Assert.assertFalse(contentSearchPage.doesContentExist(test541fileName20, test541contentStructureName20), "ERROR - Content ('"+test541fileName20+"') should exist in authoring server");
			...
			browserPage = portletMenu.getSiteBrowserPage();
			Assert.assertTrue(browserPage.doesFolderExist(test541folderName72), "ERROR - Folder ('"+test541folderName72+"') should exist in receiver server");
			folder = browserPage.editFolder(test541folderName72);
			String folderTitle = folder.getFolderTitle();
			folder.cancel();
			browserPage = portletMenu.getSiteBrowserPage();
			Assert.assertTrue(folderTitle.equals(test541folderName72), "ERROR - Folder ('"+test541folderName72+"') title doesn't match in both servers");

			browserPage.selectFolder(test541folderName72);
			Assert.assertTrue(browserPage.doesElementExist(test541fileName20), "ERROR - Folder ('"+test541folderName72+"') do not contain file asset ('"+test541fileName20+"') in receiver server");
