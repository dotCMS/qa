...
# expects QA_Milestone to be set prior to executing

env

export LASTCOMMIT=$(cat buildinfo.txt | python -c 'import sys, json; print json.load(sys.stdin)["contentlets"][0]["commitNumber"]')

curl http://dotcms.com/api/content/query/+structureName:DotcmsNightlyBuilds%20+conHost:SYSTEM_HOST%20+DotcmsNightlyBuilds.version:${QA_Milestone}/orderby/moddate%20desc/limit/1 2>/dev/null > buildinfo.txt
